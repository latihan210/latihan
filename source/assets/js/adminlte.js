...
(function (global, factory) {
	// Terapkan status sidebar SEBELUM inisialisasi AdminLTE
	try {
		var sidebarStatus = null;
		if (window.localStorage) {
			sidebarStatus = localStorage.getItem("sidebar_status");
		}
		if (sidebarStatus === "collapse") {
	...
	window._sidebarStatusSet = sidebarStatus; // Set the sidebar status flag
	...
	class PushMenu {
		...
		collapse() {
			...
			// Simpan status ke localStorage
			try {
				localStorage.setItem("sidebar_status", "collapse");
			} catch (e) {}
			this._element.dispatchEvent(event);
		}
		addSidebarBreakPoint() {
			// Jangan override jika status sudah di-set dari localStorage
			if (window._sidebarStatusSet) {
				return;
			}
	 ...
	 */
	onDOMContentLoaded(() => {
		// Baca status sidebar dari localStorage saat halaman dimuat
		/*
		try {
